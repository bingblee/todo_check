...
export default async function SummaryPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="content-page"><div className="content-wrap">
    ...
    <section className="page-card summary-card">
      <header className="summary-intro"><div><p className="eyebrow">AI REVIEW</p><h1 className="date-title">回看走过的每一步</h1><p className="muted">统计永远基于真实记录，AI 只负责帮你读懂它。</p></div><div className="summary-intro-actions"><span className="summary-period-badge">{period === "day" ? "每日回顾" : "每周回顾"}</span><GenerateSummary period={period} anchorDate={anchorDate} themeId={themeId} stale={stale} force={Boolean(content && !stale)} /></div></header>
      <div className="summary-filter-panel">
        <div className="summary-filter-row"><div><span className="summary-filter-label">统计周期</span><div className="segmented"><Link className={period === "day" ? "active" : ""} href={`/summary?period=day&date=${anchorDate}${themeId ? `&theme=${themeId}` : ""}`}>今日</Link><Link className={period === "week" ? "active" : ""} href={`/summary?period=week&date=${anchorDate}${themeId ? `&theme=${themeId}` : ""}`}>本周</Link></div></div>
          <form method="get" className="summary-date-form"><input type="hidden" name="period" value={period} />{themeId && <input type="hidden" name="theme" value={themeId} />}<label className="field">统计日期<input className="input" type="date" name="date" defaultValue={anchorDate} /></label><button className="button secondary">查看</button></form></div>
        <div className="summary-theme-filter"><span className="summary-filter-label">主题范围</span><ThemeSummarySelector userId={user.id} period={period} anchorDate={anchorDate} selected={themeId} /></div>
      </div>
      <div className="summary-hero">
        <div className="summary-hero-copy"><p className="eyebrow">{snapshot.startDate === snapshot.endDate ? snapshot.startDate : `${snapshot.startDate} — ${snapshot.endDate}`} · {themeName}</p><h2>{content?.headline ?? (snapshot.stats.planned ? "记录已经就绪" : "这段时间还没有计划")}</h2><p className="muted">{content?.overview ?? "生成总结后，DeepSeek 会结合完成情况和你的打卡感想，给出一段温和、具体的回顾。"}</p></div>
        <div className="summary-kpis"><div className="stat-card"><strong className={snapshot.stats.totalScore < 0 ? "score-negative" : ""}>{snapshot.stats.totalScore > 0 ? "+" : ""}{snapshot.stats.totalScore}</strong><span>本期总分</span></div><div className="stat-card"><strong>{snapshot.stats.completedScore > 0 ? "+" : ""}{snapshot.stats.completedScore}</strong><span>完成得分</span></div><div className="stat-card"><strong className={snapshot.stats.incompleteScore < 0 ? "score-negative" : ""}>{snapshot.stats.incompleteScore > 0 ? "+" : ""}{snapshot.stats.incompleteScore}</strong><span>未完成得分</span></div><div className="stat-card"><strong>{snapshot.stats.completionRate}%</strong><span>{snapshot.stats.scoreByDay.some((day) => day.provisional) ? "完成率 · 进行中" : "完成率"}</span></div><div className="stat-card"><strong>{snapshot.stats.completed}/{snapshot.stats.planned}</strong><span>完成 / 计划</span></div><div className="stat-card"><strong>{snapshot.stats.activeDays}</strong><span>活跃天数</span></div></div>
      </div>
...
async function ThemeSummarySelector({ userId, period, anchorDate, selected }: { userId: string; period: string; anchorDate: string; selected?: string }) {
  ...
  return <div className="theme-filter-options"><Link className={`button small ${!selected ? "" : "ghost"}`} href={`/summary?period=${period}&date=${anchorDate}`}>全部主题</Link>{themeRows.map((theme) => <Link key={theme.id} className={`button small ${selected === theme.id ? "" : "ghost"}`} href={`/summary?period=${period}&date=${anchorDate}&theme=${theme.id}`}>{theme.icon} {theme.name}</Link>)}</div>;
}
