...
export function TodayCheckins({ data }: { data: TodayData }) {
  ...
  return <main className="content-page today-page"><div className="content-wrap">
    <header className="page-nav"><div className="brand"><span className="brand-mark">✓</span><span>拾光清单</span></div><div className="inline-actions"><span className="today-count">{completed}/{total} 已完成</span><a href="/app" className="button ghost small">返回主题</a></div></header>
    <section className="page-card today-card">
      ...
      </section>)}</div> : <div className="empty-state"><h3>今天没有安排</h3><p>回到主题页，添加一个值得记录的小任务吧。</p><a className="button" href="/app">去添加任务</a></div>}
    </section>
